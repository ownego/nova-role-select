...
Nova.booting(function (Vue, router, store) {
  Vue.component('index-role-select', __webpack_require__(3));
  Vue.component('detail-role-select', __webpack_require__(6));
  Vue.component('form-role-select', __webpack_require__(9));
});
    ...
    _vm._l(_vm.roles, function(role) {
      return _c(
        "span",
        {
          key: role.id,
          staticClass:
            "whitespace-no-wrap px-2 py-1 rounded-full uppercase text-xs font-bold bg-success mr-1"
        },
        [_vm._v("\n          " + _vm._s(role.name) + "\n      ")]
      )
    }),
...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  methods: {
    ...
    setInitialValue: function setInitialValue() {
      ...
      this.value = _(this.field.roles).map(function (o) {
        var exists = _(_this.field.value).find(function (selectedRole) {
          return selectedRole.name == o.name;
        });
        var checked = !!exists;
  ...
  computed: {
    ...
    finalPayload: function finalPayload() {
      return _(this.value).filter('checked').map('name').value();
    }
  }
});
